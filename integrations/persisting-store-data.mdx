...
description: Persist your Zustand state across sessions with various storage backends
---

The Persist middleware enables you to store your Zustand state in a storage (e.g., `localStorage`, `AsyncStorage`, `IndexedDB`, etc.), thus persisting its data across browser sessions or app restarts.

<Note>
This middleware supports both synchronous storages, like `localStorage`, and asynchronous storages, like `AsyncStorage`, but using an asynchronous storage does come with a cost. See [Hydration and asynchronous storages](#hydration-and-asynchronous-storages) for more details.
</Note>
...
<ParamField path="name" type="string" required>
  The key used to store your Zustand state in the storage. Must be unique.
</ParamField>
...
<ParamField path="storage" type="() => StateStorage" default="createJSONStorage(() => localStorage)">
  The storage backend to use for persisting state.
</ParamField>
...
<ParamField path="partialize" type="(state: Object) => Object" default="(state) => state">
...
<ParamField path="onRehydrateStorage" type="(state: Object) => ((state?: Object, error?: Error) => void) | void">
  Listener function called when the storage is hydrated.
</ParamField>
...
<ParamField path="version" type="number" default="0">
  Version number for handling breaking changes in storage schema.
</ParamField>

If you want to introduce a breaking change in your storage (e.g. renaming a field), you can specify a new version number. By default, if the version in the storage does not match the version in the code, the stored value won't be used. You can use the [migrate](#migrate) function to handle breaking changes in order to persist previously stored data.

### `migrate`

<ParamField path="migrate" type="(persistedState: Object, version: number) => Object | Promise<Object>" default="(persistedState) => persistedState">
  Function to migrate persisted state between versions.
</ParamField>
...
<ParamField path="merge" type="(persistedState: Object, currentState: Object) => Object" default="(persistedState, currentState) => ({ ...currentState, ...persistedState })">
...
By default, the middleware does a shallow merge. The shallow merge might not be enough if you have partially persisted nested objects. For instance, if the storage contains:
...
<ParamField path="skipHydration" type="boolean" default="undefined">
  Skip automatic hydration on initialization.
</ParamField>

By default the store will be hydrated on initialization. In some applications you may need to control when the first hydration occurs. For example, in server-rendered apps.
...
<Note>
Available in version 3.6.3 and later.
</Note>
...
<ResponseField name="getOptions" type="() => Partial<PersistOptions>">
  Returns the current options of the Persist middleware.
</ResponseField>
...
<ResponseField name="setOptions" type="(newOptions: Partial<PersistOptions>) => void">
  Changes the middleware options. New options will be merged with the current ones.
</ResponseField>
...
<ResponseField name="clearStorage" type="() => void">
  Clears everything stored under the specified storage name.
</ResponseField>
...
<ResponseField name="rehydrate" type="() => Promise<void>">
  Manually triggers rehydration from storage.
</ResponseField>
...
<ResponseField name="hasHydrated" type="() => boolean">
  Non-reactive getter to check if the storage has been hydrated.
</ResponseField>
...
<ResponseField name="onHydrate" type="(listener: (state) => void) => () => void">
  Subscribes to hydration start events. Returns an unsubscribe function.
</ResponseField>
...
<ResponseField name="onFinishHydration" type="(listener: (state) => void) => () => void">
  Subscribes to hydration completion events. Returns an unsubscribe function.
</ResponseField>
...
<ResponseField name="createJSONStorage" type="(getStorage: () => StateStorage, options?: JsonStorageOptions) => PersistStorage">
  Helper function to create a storage object with JSON serialization.
</ResponseField>
...
The Persist middleware does two kinds of hydration: synchronous and asynchronous. If the given storage is synchronous (e.g., `localStorage`), hydration will be done synchronously. On the other hand, if the given storage is asynchronous (e.g., `AsyncStorage`), hydration will be done asynchronously.
...
If your app does depends on the persisted state at page load, see [How can I check if my store has been hydrated](#how-can-i-check-if-my-store-has-been-hydrated) in the FAQ section below.

### Usage in Next.js

Next.js uses Server Side Rendering, and it will compare the rendered component on the server with the one rendered on client. But since you are using data from browser to change your component, the two renders will differ and Next will throw a warning at you.
...
Let's say your state uses `Map` to handle a list of `transactions`, then you can convert the `Map` into an `Array` in the `storage` prop:
